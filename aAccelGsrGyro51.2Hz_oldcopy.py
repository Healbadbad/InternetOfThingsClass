...
# send the set sensors command
   ser.write(struct.pack('BBBB', 0x08, 0xC0, 0x00, 0x20))  #analog accel, gsr, MPU9150 gyro
   wait_for_ack()
...
# read incoming data
   ddata = ""
   numbytes = 0
   framesize = 21 # 1byte packet type + 2byte timestamp + 3x2byte Analog Accel + + 3x2byte MPU9150 gyro
   ...
   try:
      while True:
         ...
         (gyrox, gyroy, gyroz) = struct.unpack('>hhh', data[9:15])         (magx, magy, magz) = struct.unpack('>hhh', data[15:framesize])
